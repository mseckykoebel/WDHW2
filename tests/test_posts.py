...
class TestPostListEndpoint(unittest.TestCase):
    ...
    # honoring the limit function?
    # check that three are returned
    ...
    # what if the limit argument is bad?
    # check if less than or equal to 50, and that it is an integer
    ...
    # post returned from endpoint is something that we are authorized to see
    ...
    # create a new post (image, caption, alt text)
...
class TestPostDetailEndpoint(unittest.TestCase):
    ...
    def test_post_patch_blanks_not_overwritten(self):
        post_to_update = utils.get_post_by_user(self.current_user.get("id"))
        body = {"image_url": "https://picsum.photos/600/430?id=223"}
        url = "{0}/api/posts/{1}".format(root_url, post_to_update.get("id"))
...
if __name__ == "__main__":
    ...
    suite.addTests(
        [
            ...
            TestPostDetailEndpoint(
                "test_post_get_id_does_not_exist_404"
            ),  # get (individual)
            TestPostDetailEndpoint(
                "test_post_get_unauthorized_id_404"
            ),  # get (individual)
        ]
    )
