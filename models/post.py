...
class Post(db.Model):
    ...
    def to_dict(self, include_comments: bool = True, user=None):
        ...
        if user:
            ...
            user_like_id_list = [like.id for like in filter(
                filter_function, self.likes)]
            user_bookmark_id_list = [bookmark.id for bookmark in filter(
                filter_function, self.bookmarks)]
